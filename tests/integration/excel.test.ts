...
afterAll(async () => { await fs.rm(tempFile, { force: true }) })

describe.skipIf(skip)('Excel integration', () => {
  it('full CRUD on real xlsx file', async () => {
    ...
    const after = await connector.query(tempFile, 'Sales', [{ column: 'Name', operator: '=', value: 'IntTest' }])
